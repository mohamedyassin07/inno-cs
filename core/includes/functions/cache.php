...
function init_redis_connecting(){
    ...
    if( $redis_connection == false ){
		...
		if( $scheme && $host && $port ){
			try {
                include_once __DIR__ . '/../libs/predis/autoload.php'; 
                    $redis_connection =  new Predis\Client([
						'scheme' => $scheme ,
						'host'   => $host,
						'port'   => $port ,
					],
				...
				if ( $redis_connection->ping() != 'PONG' ) { //if not working well
					$redis_connection =  false ;
				};
...
init_redis_connecting();

function redis_key( $group , $key ){
    return  empty($group) ?  $key :  $group.".$key";
}
function convert_redis_value($value){
    // whatever what will we do on the values
    return serialize($value);
}
function revert_redis_value($value){
    return unserialize($value);
}
////////////////////////////////// Main wordpress core cashing funstions we will use 
function wp_cache_add( $key, $value, $group = '', $expiration = 0 ) {
    if( $group != 'posts'){
        return null;
    }
    $redis_connection = init_redis_connecting();
    $key = redis_key( $group, $key );
    $redis_connection->set( $key , convert_redis_value ( $value)  );
}
...
function wp_cache_get( $key, $group = '', $force = false, &$found = null ) {
    ...
    $redis_connection = init_redis_connecting();
    $key = redis_key( $group, $key );
    $post = $redis_connection->get( $key);
    $post = revert_redis_value ( $post );
    return $post;
}

////////////////////////////////// Rest of the main wordpress core functions
function wp_cache_init() {
    //
}
function wp_cache_get3( $key, $group = '', $force = false, &$found = null ) {
    
    if( $group != 'posts' ){
        return false;
    }

    if( $key == 24 && $group =='posts' ){
		global $wpdb;
        $post = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM $wpdb->posts WHERE ID = %d LIMIT 1", $key ) );
        $post = sanitize_post( $post, 'raw' );
		$post = new WP_Post( $post );

        $post_ser =  serialize( $post );
        $post_un = unserialize ( $post_ser );
        $post_un->post_content = '11111111111111111111111';
        $post_un->redis = 1;
        //print_r( $post_un );
        return $post_un;
        ...
        // $key =  redis_key( $group , $key );
        // if( $key == 'wp:default:postszzzzz.29'){
            
        //     $value = 'O:8:\"stdClass\":23:{s:2:\"ID\";s:2:\"24\";s:11:\"post_author\";s:1:\"1\";s:9:\"post_date\";s:19:\"2021-12-22 04:50:52\";s:13:\"post_date_gmt\";s:19:\"2021-12-22 01:50:52\";s:12:\"post_content\";s:73:\"<!-- wp:paragraph -->\n<p>this is the content d</p>\n<!-- /wp:paragraph -->\";s:10:\"post_title\";s:15:\"hello Redis3434\";s:12:\"post_excerpt\";s:0:\"\";s:11:\"post_status\";s:7:\"publish\";s:14:\"comment_status\";s:4:\"open\";s:11:\"ping_status\";s:4:\"open\";s:13:\"post_password\";s:0:\"\";s:9:\"post_name\";s:11:\"hello-redis\";s:7:\"to_ping\";s:0:\"\";s:6:\"pinged\";s:0:\"\";s:13:\"post_modified\";s:19:\"2021-12-22 05:16:22\";s:17:\"post_modified_gmt\";s:19:\"2021-12-22 02:16:22\";s:21:\"post_content_filtered\";s:0:\"\";s:11:\"post_parent\";s:1:\"0\";s:4:\"guid\";s:18:\"http://redis/?p=24\";s:10:\"menu_order\";s:1:\"0\";s:9:\"post_type\";s:4:\"post\";s:14:\"post_mime_type\";s:0:\"\";s:13:\"comment_count\";s:1:\"0\";}';
...
function wp_cache_get2( $key, $group = '', $force = false, &$found = null ) {
}
